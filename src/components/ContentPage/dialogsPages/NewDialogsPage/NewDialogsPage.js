...
const NewDialogsPage = () => {
	...
	const debounceFilterInput = debounce(
		(e) => setFilterInput(e.target.value),
		400
	)
	//
	...
	const handleAddNewDialog = () => {
		...
		set(newDialogRef, {
			dialogId:
				newDialogRef._path.pieces_[
					Object.keys(newDialogRef._path.pieces_).length - 1
				],
			clientName: clientName,
			startTime: date.getTime()
		})
		const firstMessageListRef = ref(
			dataBase,
			`newDialogs/${
				newDialogRef._path.pieces_[
					Object.keys(newDialogRef._path.pieces_).length - 1
		...
		set(firstMessageRef, {
			messageId:
				firstMessageRef._path.pieces_[
					Object.keys(firstMessageRef._path.pieces_).length - 1
				],
			text: firstMessage,
	...
	const handleAddToActiveDialogs = (
		clientName,
		dialogId,
		startTime,
		messages
		...
		const startedActiveDialogsIdRef = ref(
			dataBase,
			`users/${id}/startedActiveDialogsId/${dialogId}`
		)
		set(startedActiveDialogsIdRef, {
			dialogId
		})
		if (autoGreeting) {
			const autoGreetingListRef = ref(
				dataBase,
				`activeDialogs/${dialogId}/messages`
			)
			const autoGreetingRef = push(autoGreetingListRef)
			set(autoGreetingRef, {
				messageId:
					autoGreetingRef._path.pieces_[
						Object.keys(autoGreetingRef._path.pieces_).length - 1
					],
				text: autoGreeting,
				timestamp: date.getTime(),
				senderName: email
		...
		<>
			<div className={s.header}>
				...
				<div className={s.input}>
					Поиск по:
					<ButtonGroup>
						<Button color="secondary" onClick={() => setRadioButton('имени')}>
							Имени
						</Button>
						<Button
							color="secondary"
					...
					<Input
						placeholder={`поиск по ${radioButton}`}
