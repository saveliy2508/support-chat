...
const SavedDialogsPage = () => {
	...
	return (
		<div className={s.savedDialogs}>
			<div className={s.title}>SavedDialogsPage</div>
			<div className={s.dialogsCards}>
				{savedDialogsId && activeDialogs
					? activeDialogs
							.filter((activeDialogItem) =>
								Object.values(savedDialogsId).find((savedDialogId) =>
									activeDialogItem.dialogId === savedDialogId.dialogId
										? true
										: null
								)
							)
