...
import {setDialogsSettings, setAutoGreening} from "../../../../redux/actions/userActions";

const DialogsSettings = () => {
    const {id, templatePhrases, autoGreeting} = useSelector(state => state.user)
    ...
    const updateDialogsSettings = async (values) => {
        ...
        await values.templatePhrases.forEach(item => {
            if (item !== '') {
                ...
                set(newPhraseRef, {
                    phraseId: newPhraseRef._path.pieces_[Object.keys(newPhraseRef._path.pieces_).length - 1],
                    text: item,
                })
            }
        })
        await set(ref(dataBase, `users/${id}/autoGreeting`), autoGreeting);
        dispatch(setDialogsSettings({
    ...
    return (
        <>
            <div className={s.greeding}>
                ...
                <Input value={autoGreeting} onChange={e => dispatch(setAutoGreening(e.target.value))}/>
            </div>
            <div className={s.phrases}>
                ...
                >
                    {({values}) => (<Form className={s.phrasesForm}>
                        <FieldArray name='templatePhrases'>
                            {fieldArrayProps => {
                                ...
                                return <div>
                                    ...
                                    <Button onClick={() => push('')} className={s.addButton}>Добавить фразу</Button>
                                    {templatePhrases && templatePhrases.map(((phrase, index) => (
                                        <div className={s.phrase} key={index}>
                                            <Field name={`templatePhrases[${index}]`} value={phrase}>
                                                {({field}) => (<>
                                                    <InputGroup>
                                                        <Input {...field}/>
