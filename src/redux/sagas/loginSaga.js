...
export function* loginFunctionWorker({ response }) {
	...
	let savedDialogsId
	const savedDialogsIdRef = yield ref(
		dataBase,
		`users/${user.uid}/savedDialogsId`
	)
	yield get(savedDialogsIdRef).then((snapshot) => {
		savedDialogsId = snapshot.val()
	})

	let startedActiveDialogsId
	const startedActiveDialogsRef = yield ref(
		dataBase,
		`users/${user.uid}/startedActiveDialogsId`
	)
	yield get(startedActiveDialogsRef).then((snapshot) => {
	...
	let newDialogs = []
	yield get(newDialogsRef).then((snapshot) => {
	...
	const activeDialogsRef = yield query(
		ref(dataBase, 'activeDialogs'),
		orderByChild('operatorId'),
		equalTo(user.uid)
	)
	let activeDialogs = []
	yield get(activeDialogsRef).then((snapshot) => {
		snapshot.forEach((childSnapshot) => {
			const data = childSnapshot.val()
